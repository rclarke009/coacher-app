...
class ReminderManager: ObservableObject {
    ...
    private func scheduleNightPrepReminder() async {
        ...
        do {
            ...
            print("🔍 DEBUG: ReminderManager - Scheduled night prep reminder for \(components.hour ?? 0):\(components.minute ?? 0)")
        } catch {
    ...
    private func scheduleMorningFocusReminder() async {
        ...
        do {
            try await center.add(request)
            print("🔍 DEBUG: ReminderManager - Scheduled morning focus reminder for \(components.hour ?? 0):\(components.minute ?? 0)")
        } catch {
